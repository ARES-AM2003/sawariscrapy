#!/usr/bin/env python3
"""
Automatic Fragment Pipeline Configuration and Execution Script
...
4. Runs the complete fragment pipeline automatically (uses run_feat_next.py instead of run_feature_spec_parallel.py)

Usage:
    1. Update the variables in the CONFIGURATION section below
    2. Run: python3 auto_update_and_run_fragment.py

The script will:
    - Update all spider files with brand/model names and URLs
    - Update link_and_path.py
    - Create the output folder structure
    - Run the complete fragment pipeline automatically
"""
...
CARDEKHO_LINK = "https://www.cardekho.com/mahindra/xuv700"
CARWALE_LINK = "https://www.carwale.com/mahindra-cars/xuv700/"
AUTOCARINDIA_LINK = "https://www.autocarindia.com/cars/mahindra/xuv700"
...
class FragmentPipelineUpdater:
    """Handles updating all spider files and running the fragment pipeline."""
    ...
    def __init__(self, brand, model, cardekho_url, carwale_url, autocar_url):
        ...
        self.fragment_pipeline_script = os.path.join(
            self.script_dir, "run_fragment_complete.py"
        )
...
'''
    ...
    def run_fragment_pipeline(self):
        """Run the complete fragment pipeline."""
        self.log("Starting fragment pipeline execution...")

        if not os.path.exists(self.fragment_pipeline_script):
            self.log(
                f"✗ Fragment pipeline script not found: {self.fragment_pipeline_script}",
                "ERROR",
            )
            return False

        try:
            # Run the fragment pipeline
            self.log(
                f"Executing: python3 {self.fragment_pipeline_script} {self.output_folder}"
            )
            result = subprocess.run(
                ["python3", self.fragment_pipeline_script, self.output_folder],
            ...
            if result.returncode == 0:
                self.log("✓ Fragment pipeline completed successfully!", "SUCCESS")
                return True
            else:
                self.log(
                    f"✗ Fragment pipeline failed with exit code {result.returncode}",
                    "ERROR",
                )
                return False

        except Exception as e:
            self.log(f"✗ Error running fragment pipeline: {e}", "ERROR")
            return False
    ...
    def display_config(self):
        ...
        print(f"Pipeline Type:     FRAGMENT (uses run_feat_next.py)")
        print(f"\nURLs:")
    ...
    def run(self):
        """Execute the complete update and fragment pipeline run process."""
        start_time = datetime.now()

        self.print_header("AUTO SPIDER UPDATER AND FRAGMENT PIPELINE RUNNER")
        ...
        # Step 4: Run fragment pipeline
        self.print_header("STEP 4: Running Complete Fragment Pipeline")
        self.log("This may take several minutes depending on the number of variants...")
        self.log(
            "NOTE: This pipeline uses run_feat_next.py for feature/spec extraction"
        )

        pipeline_success = self.run_fragment_pipeline()
        ...
        print(f"\nPipeline Type:  FRAGMENT (run_feat_next.py)")
...
╔══════════════════════════════════════════════════════════════════════════════╗
║                                                                              ║
║       AUTO SPIDER UPDATER AND FRAGMENT PIPELINE RUNNER                      ║
║       Automatically configure spiders and run fragment pipeline              ║
║       (uses run_feat_next.py instead of run_feature_spec_parallel.py)       ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
    """)
    ...
    updater = FragmentPipelineUpdater(
        brand=BRAND_NAME,
        model=MODEL_NAME,
